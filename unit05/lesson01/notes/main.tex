%! 3 by 3 Determinants — Unit 5, Lesson 5.1 — Guided Notes
...
\vspace{2pt}
\termblanklong{Minor (cross out a row \& column)}
\termblanklong{Cofactor expansion ($+\;-\;+$ signs)}
\termblanklong{Signed volume (parallelepiped)}
...
\begin{itemize}\setlength{\itemsep}{1pt}
  \item What \emph{one} number should measure how much space that box fills? \writeline
  \item If the three columns all lie flat in one plane, what is the box's volume? \writeline
\end{itemize}
\end{hookbox}
...
\end{tikzpicture}
\end{center}
The three edges from the corner are the three \blank{2.2cm} of $A$; the determinant packs the
...
\[
  \det A = 2\det\begin{bmatrix} 3 & 1 \\ 1 & 2 \end{bmatrix}
         ...
         + 0
         = 2(\blank{0.9cm}) - 1(\blank{0.9cm}) = \blank{1.2cm}.
\]
...
\[
  \det\begin{bmatrix} 2 & 0 & 0 \\ 0 & 3 & 0 \\ 0 & 0 & 4 \end{bmatrix} = \blank{1.2cm}
...
collapses onto a plane, and a collapse cannot be \blank{2.2cm}. So one number decides everything:
\[
  \det A \ne 0 \;\Leftrightarrow\; \text{columns independent} \;\Leftrightarrow\; A \text{ is }\blank{2.2cm}.
\]
...
$|\det A|$ is the factor every \emph{volume} is \blank{2.2cm} by.
\end{notesbox}
...
\begin{enumerate}[leftmargin=1.6em, itemsep=6pt]
  \item Compute $\det\begin{bsmallmatrix} 1 & 2 & 0 \\ 0 & 3 & 1 \\ 1 & 0 & 2 \end{bsmallmatrix}$. \writeline
  \item Find the volume of the box with columns $\begin{bsmallmatrix} 4\\0\\0 \end{bsmallmatrix}$, $\begin{bsmallmatrix} 0\\3\\0 \end{bsmallmatrix}$, $\begin{bsmallmatrix} 0\\0\\2 \end{bsmallmatrix}$. \writeline
  \item The three columns of a $3\times3$ matrix lie in one plane. What is $\det$? Is the matrix invertible? \writeline
\end{enumerate}
\end{practicebox}
