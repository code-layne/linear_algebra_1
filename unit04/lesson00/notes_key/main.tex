%! Right Angles Revisited --- Setting Up Orthogonality — Unit 4, Lesson 4.0 — Guided Notes (Answer Key)
...
\vspace{2pt}
\vocabans{Dot product $\vv\cdot\ww$}{multiply matching components and add: $v_1w_1+v_2w_2+\cdots$; one number whose sign gives the angle}
\vocabans{Length $\|\vv\|$ and unit vector}{$\|\vv\|=\sqrt{\vv\cdot\vv}$; the unit vector $\vv/\|\vv\|$ has the same direction and length $1$}
\vocabans{Orthogonal (perpendicular)}{two vectors meeting at a right angle; the test is $\vv\cdot\ww=0$}
\vocabans{Orthogonal complement (recall §3.6)}{two subspaces that are perpendicular and together fill the whole space (e.g.\ $N(A)$ and the row space)}
\vocabans{Projection (preview)}{the closest point of a line/subspace to a target --- the foot of the perpendicular}
\end{vocabbox}

\begin{hookbox}
A hiker stands \emph{off} a straight trail and wants the shortest walk back to it.
\begin{itemize}\setlength{\itemsep}{1pt}
  \item Of all the routes from the hiker back to the trail, which is \emph{shortest}? \ansline{The one that meets the trail straight on --- the perpendicular route.}
  \item What is the \emph{angle} between that shortest route and the trail? \ansline{A right angle ($90^\circ$).}
\end{itemize}
\end{hookbox}
...
\[
  \vv\cdot\ww = v_1w_1 + v_2w_2 + \cdots + v_nw_n \quad(\text{one \ans{number}}).
\]
Its \textbf{sign} reports the angle: positive $=$ \ans{acute}, zero $=$ a \ans{right} angle,
negative $=$ obtuse.

\vspace{3pt}
The \textbf{length} (or norm) is $\|\vv\|=\sqrt{\vv\cdot\vv}$. For
$\vv=\begin{bsmallmatrix} 3\\4 \end{bsmallmatrix}$:
$\|\vv\|=\sqrt{3^2+4^2}=\ans{$5$}$. To make a \textbf{unit vector} (same direction, length~1),
\ans{divide} by the length: $\dfrac{\vv}{\|\vv\|}=\begin{bsmallmatrix} 3/5\\4/5 \end{bsmallmatrix}$.
\end{notesbox}
...
\[
  \vv\cdot\ww = {\color{keyred}\mathbf{0}}.
\]
...
= 12 + ({\color{keyred}\mathbf{-12}}) = 0$, so they are \ans{perpendicular}.
...
gives $0$. So every row is \ans{orthogonal (perpendicular)} to $\xx$ --- that is just
$\vv\cdot\ww=0$ again. Try it on
...
= -2 + 2 = \ans{$0$}$. \checkmark\ Orthogonality is the engine of the whole picture.
\end{notesbox}
...
the perpendicular. The gap $\bb-\pp$ (the \emph{error}) is \ans{perpendicular} to the line.
...
move: \emph{make something} \ans{perpendicular}.
\end{notesbox}

\begin{practicebox}
Use $\vv\cdot\ww=0$ for perpendicular. Show your dot products.
\begin{enumerate}[leftmargin=1.6em, itemsep=4pt]
  \item Are $\begin{bsmallmatrix} 5\\2 \end{bsmallmatrix}$ and $\begin{bsmallmatrix} 2\\-5 \end{bsmallmatrix}$ perpendicular? \ansline{Yes --- $(5)(2)+(2)(-5)=10-10=0$.}
  \item Find the length and unit vector of $\begin{bsmallmatrix} 6\\8 \end{bsmallmatrix}$. \ansline{$\|\cdot\|=\sqrt{36+64}=\sqrt{100}=10$; unit vector $\begin{bsmallmatrix} 6/10\\8/10 \end{bsmallmatrix}=\begin{bsmallmatrix} 3/5\\4/5 \end{bsmallmatrix}$.}
  \item Find $x$ so that $\begin{bsmallmatrix} 4\\x \end{bsmallmatrix}$ is perpendicular to $\begin{bsmallmatrix} 3\\6 \end{bsmallmatrix}$. \ansline{$4(3)+x(6)=0\Rightarrow 12+6x=0\Rightarrow x=-2$.}
\end{enumerate}
\end{practicebox}

\begin{teachernote}
The whole unit rests on Section~2: \emph{dot $=0$ $\Leftrightarrow$ perpendicular}. Sections~3 and~4
show the two big payoffs --- the §3.6 subspace pairs are perpendicular \emph{because} of dot $=0$
(read $A\xx=\zero$ row by row), and the closest-point/projection idea is dropping a perpendicular.
Most common errors: forgetting the square root in $\|\vv\|$, and reading a negative dot product as
``perpendicular.'' Only \emph{zero} is a right angle.
\end{teachernote}
