%! Piecewise Linear Learning Functions — Unit 8, Lesson 8.1 — Guided Notes
...
\termblanklong{Fold (corner) --- where the slope changes}
\termblanklong{Shift $\relu(x-s)$ --- moves a fold to $x=s$ (the bias)}
\termblanklong{Weight on a ReLU --- how much slope it adds}
\termblanklong{Counting rule: $N$ folds $\Rightarrow N+1$ straight pieces}
\end{vocabbox}
...
\begin{itemize}\setlength{\itemsep}{1pt}
  \item A single straight line $y=wx+b$ has one slope forever. To trace data that rises \emph{and} falls, what does the graph need that a line does not have? \writeline
  \item If replacing $x$ with $x-2$ inside $\relu$ slides its fold from $x=0$ to $x=2$, what part of a layer ($A\vv+\bb$) do you think controls \emph{where} a fold sits? \writeline
\end{itemize}
\end{hookbox}
...
\[
  F(0)=\blank{0.9cm},\qquad F(2)=\blank{0.9cm},\qquad F(4)=\blank{0.9cm},\qquad F(6)=\blank{0.9cm}.
\]
...
\[
  ...
  \underbrace{0<x<2}_{+\relu(x)}:\ \text{slope}=\blank{0.8cm}.
\]
\[
  \underbrace{2<x<4}_{-2\,\relu(x-2)}:\ \text{slope}=1-2=\blank{0.8cm},\qquad
  \underbrace{x>4}_{+\relu(x-4)}:\ \text{slope}=\blank{0.8cm}.
\]
...
\vspace{2pt}
\textbf{Counting rule.} Three ReLUs gave three folds and \blank{0.9cm} straight pieces. In general,
\[
...
So a layer with $N$ rows (ReLUs) builds a piecewise-linear function with \blank{1.6cm} folds --- one per row.
...
\begin{enumerate}[leftmargin=1.6em, itemsep=4pt]
  \item For $G(x)=\relu(x)-\relu(x-2)$: where are the folds, what is the slope on each of the three pieces, and what are $G(1)$ and $G(3)$? \writeline
  \item A piecewise-linear function is built from $4$ ReLUs. How many folds does it have, and how many straight pieces? \writeline
  \item For the tent $F(x)=\relu(x)-2\,\relu(x-2)+\relu(x-4)$, what is $F(2)$, and what is the slope of the piece just after $x=2$? \writeline
\end{enumerate}
\end{practicebox}
