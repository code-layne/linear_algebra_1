%! Independence, Basis, and Dimension — Unit 3, Lesson 3.4 — Guided Notes
...
\vspace{2pt}
\termblanklong{Linearly independent}
\termblanklong{Linearly dependent}
\termblanklong{Basis}
\termblanklong{Dimension}
\end{vocabbox}
...
column~2).
\begin{itemize}\setlength{\itemsep}{1pt}
  \item How many levers are \emph{really} doing something new? \writeline
  \item So how many independent directions can the tip move in? \writeline
\end{itemize}
\end{hookbox}
...
\[
  c_1\vv_1+c_2\vv_2+\dots+c_k\vv_k=\zero \quad\Longrightarrow\quad \text{every } c_i=\blank{0.7cm}.
\]
If instead \emph{some} combination with not-all-zero weights gives $\zero$, they are \textbf{dependent} ---
and then at least one vector is a \blank{2.6cm} of the others (it is redundant).
\begin{minipage}[t]{0.60\linewidth}\vspace{3pt}
\textbf{Geometry.} Two vectors are dependent exactly when they lie on one \blank{1.4cm} (one is a multiple
...
the columns are \blank{2.4cm}. Setting the free variable to $1$ gives
$\svec=\begin{bmatrix}\rule{0pt}{1.0em}\blank{0.7cm}\\ \blank{0.7cm}\\ \blank{0.7cm}\end{bmatrix}$, which
...
\begin{itemize}\setlength{\itemsep}{2pt}
  \item \textbf{Basis for $C(A)$:} the \blank{2.2cm} columns of $A$ (the \emph{original} columns, not the reduced ones). Above: $\begin{bsmallmatrix} 1\\1 \end{bsmallmatrix},\begin{bsmallmatrix} 1\\2 \end{bsmallmatrix}$.
  \item \textbf{Basis for $N(A)$:} the \blank{2.2cm} solutions (one per free column). Above: $\svec=\begin{bsmallmatrix} -1\\-1\\1 \end{bsmallmatrix}$.
\end{itemize}
...
\[
  \dim C(A)=r=\blank{0.7cm}, \qquad \dim N(A)=n-r=\blank{0.7cm}, \qquad r+(n-r)=n=\blank{0.7cm}.
\]
...
\begin{enumerate}[leftmargin=1.6em, itemsep=2pt]
  \item Are $\begin{bsmallmatrix} 1\\3 \end{bsmallmatrix}$ and $\begin{bsmallmatrix} 2\\6 \end{bsmallmatrix}$ independent? Say how you know. \writeline
  \item For $A=\begin{bmatrix} 1 & 2 & 1 \\ 0 & 0 & 1 \end{bmatrix}$, give a basis for $C(A)$ and its dimension, and a basis for $N(A)$ and its dimension. \writeline
  \item Can $4$ vectors in $\mathbb{R}^3$ ever be independent? Explain in one sentence. \writeline
\end{enumerate}
\end{practicebox}
