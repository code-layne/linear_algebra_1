%! Diagonalizing a Matrix — Unit 6, Lesson 6.2 — Guided Notes
...
\vspace{2pt}
\termblanklong{Eigenvector matrix $X$ (eigenvectors in the columns)}
\termblanklong{Eigenvalue matrix $\Lambda$ (eigenvalues down the diagonal)}
\termblanklong{Diagonalization $A=X\Lambda X^{-1}$}
\termblanklong{Power rule $A^k=X\Lambda^k X^{-1}$}
\end{vocabbox}
...
\begin{itemize}\setlength{\itemsep}{1pt}
  \item Suppose you needed $A^{10}$. Would you really multiply $A$ by itself ten times? \writeline
  \item Along an eigenvector, $A$ acts like a single \emph{number} $\lambda$. Could we make the whole matrix act that simply? \writeline
\end{itemize}
\end{hookbox}
...
\]
So lining the eigenvectors up gives the clean identity $AX=\blank{1.4cm}$. (Scaling the columns of $X$
by the eigenvalues is exactly what multiplying by $\Lambda$ on the \blank{1.3cm} does.)
\end{notesbox}
...
by $X^{-1}$:
\[
  AX X^{-1}=X\Lambda X^{-1}\quad\Longrightarrow\quad A=\blank{2.4cm}.
\]
...
\[
  ...
  =\begin{bmatrix}\rule{0pt}{1.1em}\blank{0.7cm} & \blank{0.7cm}\\[3pt]\blank{0.7cm} & \blank{0.7cm}\end{bmatrix}=A.\quad\checkmark
\]
\end{notesbox}
...
$A^2=X\Lambda^2 X^{-1}=\begin{bsmallmatrix}\blank{0.5cm}&\blank{0.5cm}\\[2pt]\blank{0.5cm}&\blank{0.5cm}\end{bsmallmatrix}$.
...
\begin{itemize}\setlength{\itemsep}{2pt}
  ...
  \item \textbf{A repeat can fail.} The shear $\begin{bsmallmatrix}2&1\\0&2\end{bsmallmatrix}$ has only $\lambda=2$
        and \emph{one} eigenvector direction $\begin{bsmallmatrix}1\\0\end{bsmallmatrix}$ --- not enough to build $X$, so it \blank{2.6cm} be diagonalized.
\end{itemize}
...
\begin{enumerate}[leftmargin=1.6em, itemsep=6pt]
  \item $A=\begin{bsmallmatrix} 3 & 1\\ 0 & 2 \end{bsmallmatrix}$ has $\lambda=3,2$ with eigenvectors $\begin{bsmallmatrix}1\\0\end{bsmallmatrix}$ and $\begin{bsmallmatrix}1\\-1\end{bsmallmatrix}$. Write $X$, $\Lambda$, and $X^{-1}$. \writelines{2}
  \item Using part 1, write $A^2$ as $X\Lambda^2 X^{-1}$ and multiply it out. \writeline
  \item Why does the shear $\begin{bsmallmatrix}2&1\\0&2\end{bsmallmatrix}$ fail to diagonalize? \writeline
\end{enumerate}
\end{practicebox}
