%! Sets of Vectors and the Road to Subspaces — Unit 3, Lesson 3.0 — Guided Notes
...
\vspace{2pt}
\termblanklong{Vector space $\mathbb{R}^n$}
\termblanklong{Span (of some vectors)}
\termblanklong{Subspace}
\termblanklong{Closed under addition / scaling}
...
\begin{itemize}\setlength{\itemsep}{1pt}
  \item Which points can it reach with $c\vv+d\ww$? Describe the set. \writeline
  \item Now $\ww$ becomes $\begin{bsmallmatrix} 4\\2 \end{bsmallmatrix}=2\vv$. What can it reach now? \writeline
\end{itemize}
\end{hookbox}
...
components. So $\mathbb{R}^2$ is the whole \blank{2.4cm}, and $\mathbb{R}^3$ is all of space.
...
\begin{itemize}[leftmargin=1.5em, itemsep=1pt]
  \item a \textbf{line} through the origin --- when the vectors point along \blank{3.0cm};
  \item a \textbf{plane} through the origin --- when they point in \blank{3.4cm} directions;
  \item \textbf{all of $\mathbb{R}^n$}.
\end{itemize}
Every span passes through the origin, because taking all weights $=0$ gives the \blank{1.8cm}
vector.
\end{notesbox}
...
\emph{if a set does not contain $\zero$, it is} \blank{3.2cm} \emph{a subspace}.
\end{notesbox}
...
\begin{itemize}[leftmargin=1.5em, itemsep=2pt]
  \item \textbf{Line missing $\zero$:} it does not contain \blank{1.6cm}; and scaling a point on
  ...
  \item \textbf{First quadrant:} it \emph{is} closed under adding, but scaling
        $\begin{bsmallmatrix} 1\\1 \end{bsmallmatrix}$ by $c=\blank{1.0cm}$ gives
...
all of $\mathbb{R}^2$. (In $\mathbb{R}^3$, add every \blank{2.6cm} through the origin.) Every
...
is still a combination of the columns --- still \blank{2.6cm}. So $C(A)$ passes the closure test.

\vspace{3pt}
This renames a Unit~2 idea: ``$A\mathbf x=\mathbf b$ has a solution exactly when $\mathbf b$ is
reachable'' becomes ``\dots exactly when $\mathbf b$ is in the subspace \blank{2.0cm}.''
...
\begin{enumerate}[leftmargin=1.6em, itemsep=4pt]
  \item All vectors on the line $y=3x$ (i.e.\ all multiples of $\begin{bsmallmatrix} 1\\3 \end{bsmallmatrix}$). \writeline
  \item All vectors on the line $y=3x+2$. \writeline
  \item All of $\mathbb{R}^2$. \writeline
\end{enumerate}
\end{practicebox}
