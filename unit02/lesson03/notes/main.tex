%! Matrix Computations and A = LU — Unit 2, Lesson 2.3 — Guided Notes
...
\vspace{2pt}
\termblanklong{Lower triangular $L$}
\termblanklong{Upper triangular $U$}
\termblanklong{Factorization $A=LU$}
\termblanklong{Forward substitution}
\termblanklong{Two triangular sweeps}
\end{vocabbox}
...
\begin{itemize}\setlength{\itemsep}{1pt}
  \item In 2.2 we built the whole inverse $A^{-1}$. But elimination in 2.1 already did the hard part.
        What if we could just \emph{save} that work and reuse it for every order? \writeline
\end{itemize}
\end{hookbox}
...
\[
  L = \begin{bmatrix} 1 & 0 \\ \blank{0.8cm} & 1 \end{bmatrix}.
\]
...
\[
  ...
  = \begin{bmatrix}\rule{0pt}{1.1em}\blank{0.8cm} & \blank{0.8cm}\\[2pt]\blank{0.8cm} & \blank{0.8cm}\end{bmatrix} = A. \quad\checkmark
\]
...
\[
  \begin{array}{ll}
    ...
    \ell_{31} = 3 \div 1 = \blank{0.7cm}, & \text{(clears the $(3,1)$ entry)}\\[3pt]
    \ell_{32} = 1 \div 1 = \blank{0.7cm}. & \text{(clears the $(3,2)$ entry)}
  \end{array}
\]
...
\[
  L = \begin{bmatrix} 1 & 0 & 0 \\ \ell_{21} & 1 & 0 \\ \ell_{31} & \ell_{32} & 1 \end{bmatrix}
    = \begin{bmatrix} 1 & 0 & 0 \\ \blank{0.7cm} & 1 & 0 \\ \blank{0.7cm} & \blank{0.7cm} & 1 \end{bmatrix}.
\]
...
\[
  ...
  \;\Rightarrow\; c_1 = \blank{0.8cm},\quad 2c_1 + c_2 = 15 \Rightarrow c_2 = \blank{0.8cm}.
\]
So $\cc=\begin{bsmallmatrix} 7\\1 \end{bsmallmatrix}$ --- exactly the updated right-hand side from 2.1!
...
\[
  ...
  \;\Rightarrow\; y = \blank{0.8cm},\quad x + 3y = 7 \Rightarrow x = \blank{0.8cm}.
\]
So $(x,y)=(\,\blank{1.4cm}\,)$ --- the same answer as 2.1 and 2.2, with no inverse in sight.
\end{notesbox}
...
\begin{enumerate}[leftmargin=1.6em]
  \item Find the multiplier, then write $L$ and $U$. \hfill $\ell=\blank{1.0cm}$
  \item \textbf{Check} $LU=A$. \writeline
  \item Solve $A\xx=\bb$ by two sweeps ($L\cc=\bb$, then $U\xx=\cc$). \hfill $x=\blank{1.2cm}$\quad $y=\blank{1.2cm}$
\end{enumerate}
\end{practicebox}
