%! Linear Combinations of Vectors — Unit 1, Lesson 1.1 — Group Activity (Answer Key)
...
\begin{enumerate}[leftmargin=1.7em, itemsep=8pt]
  \item $\vv + \ww$
\begin{work}
  \vv+\ww &= \begin{bsmallmatrix} 2+1 \\ 1+3 \end{bsmallmatrix}
           = \begin{bsmallmatrix} 3 \\ 4 \end{bsmallmatrix}
           \ \ \text{: the point } (3,4)
\end{work}
  \item $2\vv$ (use move $\vv$ twice)
\begin{work}
  2\vv &= \begin{bsmallmatrix} 2(2) \\ 2(1) \end{bsmallmatrix}
...
\begin{work}
  \vv-\ww &= \begin{bsmallmatrix} 2-1 \\ 1-3 \end{bsmallmatrix}
           ...
           \ \ \text{: the point } (1,-2)
\end{work}
  \item If the robot uses $0$ of each move ($0\,\vv + 0\,\ww$), where is it? Why does that make sense?
        \ansline{At the origin $(0,0)$ --- it never moved, so it stays where it started.}
\end{enumerate}
\end{tcolorbox}
...
\begin{work}
  ...
  c+3d &= 6 \\
     c &= 3,\ d = 1 \\
  3\vv+\ww &= \begin{bsmallmatrix} 6 \\ 3 \end{bsmallmatrix}
              + \begin{bsmallmatrix} 1 \\ 3 \end{bsmallmatrix}
            = \begin{bsmallmatrix} 7 \\ 6 \end{bsmallmatrix} \ \ \text{\checkmark}
\end{work}
  \item A \emph{second} robot has broken moves $\mathbf{a} = \begin{bmatrix} 1 \\ 2 \end{bmatrix}$
        ...
        $c\,\mathbf{a} + d\,\mathbf{b}$. What goes wrong?
        \ansline{$\mathbf{b}=2\mathbf{a}$, so every combination is a multiple of $\mathbf{a}$ ---
        it stays on the line $y=2x$. The point $(3,0)$ is not on that line, so it is unreachable.}
  \item \textbf{Interpret.} The first robot can reach any target, but the second cannot.
        What is different about its two moves?
        \ansline{The first robot's moves are \emph{not} parallel, so they span the whole plane.
        The second robot's moves are parallel (same line), so they span only a line.}
\end{enumerate}
\end{tcolorbox}
...
\begin{enumerate}[leftmargin=1.7em, itemsep=8pt]
  \item For each pair, decide whether its combinations span the \textbf{whole plane} or only a
        ...
        \end{enumerate}
        \ansline{(a) Line --- $\begin{bsmallmatrix} 4\\6 \end{bsmallmatrix}=2\begin{bsmallmatrix} 2\\3 \end{bsmallmatrix}$ (parallel).
        \ (b) Plane --- not parallel.
        \ (c) Line --- $\begin{bsmallmatrix} -6\\2 \end{bsmallmatrix}=-2\begin{bsmallmatrix} 3\\-1 \end{bsmallmatrix}$ (parallel).}
  \item \textbf{Justify.} Explain why two \emph{parallel} vectors can never span the whole
        plane, no matter what weights you choose.
        \ansline{If $\ww=k\vv$, then $c\vv+d\ww=(c+dk)\vv$ --- always a single multiple of $\vv$.
        Every result lands on the one line through $\vv$, so points off that line are unreachable.}
\end{enumerate}
\end{tcolorbox}
