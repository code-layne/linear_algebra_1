...
\begin{enumerate}[leftmargin=1.7em, itemsep=10pt]
  \item Multiply \emph{by columns}: compute $C\begin{bsmallmatrix} 1\\0 \end{bsmallmatrix}$ and
        $C\begin{bsmallmatrix} 0\\1 \end{bsmallmatrix}$ (the first two columns of the catalog). \vspace{0.6cm}
  \item Compute the third catalog product $C\begin{bsmallmatrix} 1\\2 \end{bsmallmatrix}=1\,\mathbf{c}_1+2\,\mathbf{c}_2$. \vspace{0.7cm}
  \item Write out the full catalog $A=CR$. What does column~3 mean --- how is that product built
        from the base products? \writeline
\end{enumerate}
\end{tcolorbox}
...
\begin{enumerate}[leftmargin=1.7em, itemsep=10pt]
  ...
  \item Show product~3 is a bundle: find weights so $\begin{bsmallmatrix} 4\\5 \end{bsmallmatrix}
        ...
        Use them to write $R$. \vspace{0.7cm}
  \item \textbf{Interpret.} State $A=CR$ and the \textbf{rank}. What does the rank say about how many
        \emph{truly different} products this catalog really has? \writelines{2}
\end{enumerate}
\end{tcolorbox}
...
\begin{enumerate}[leftmargin=1.7em, itemsep=10pt]
  \item Show all three columns are multiples of $\begin{bsmallmatrix} 1\\2 \end{bsmallmatrix}$. What
        is the \textbf{rank} of $M$? \writeline
  \item Write $M=CR$ with $C$ a single column and $R$ a single row. \vspace{0.9cm}
  \item \textbf{Justify.} Explain why \emph{every} product in this catalog must use twice as much
        labor as parts --- no matter which base you pick. \writelines{2}
\end{enumerate}
\end{tcolorbox}
