...
\begin{enumerate}[leftmargin=1.7em, itemsep=8pt]
  \item Write the elimination matrix $E_{21}$ (put $-\ell$ in the lower-left of $I$).
        \; $E_{21}=\begin{bmatrix} 1 & 0 \\ {\color{keyred}\mathbf{-2}} & 1 \end{bmatrix}$ \vspace{0.2cm}
  \item Multiply $E_{21}A$ to reach upper-triangular $U$.
\begin{work}
  -2(2,5)+(4,12) &= (0,2) \\
  E_{21}A &= \begin{bsmallmatrix} 2&5\\0&2 \end{bsmallmatrix} = U
\end{work}
  \item Write $E_{21}^{-1}$, the matrix that \emph{undoes} the step. \; $E_{21}^{-1}=\begin{bmatrix} 1 & 0 \\ {\color{keyred}\mathbf{2}} & 1 \end{bmatrix}$ \vspace{0.2cm}
...
\end{work}
\ansline{The formula divides by zero and elimination hits a zero pivot, so $A$ \textbf{is
singular} --- no inverse. Its columns are parallel (column~2 $=1.5\times$ column~1).}
\end{tcolorbox}
