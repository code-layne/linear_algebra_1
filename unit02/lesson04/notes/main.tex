%! Permutations and Transposes — Unit 2, Lesson 2.4 — Guided Notes
...
\vspace{2pt}
\termblanklong{Permutation matrix $\PP$}
\termblanklong{Zero pivot / row exchange}
\termblanklong{Factorization $\PP A=LU$}
\termblanklong{Transpose $A^{\tp}$}
\termblanklong{Symmetric matrix}
\end{vocabbox}
...
\begin{itemize}\setlength{\itemsep}{1pt}
  \item The first pivot is the top-left entry --- but it is $0$. Elimination can't start. What simple
        move rescues the system? \writeline
\end{itemize}
\end{hookbox}
...
\[
  ...
  \qquad
  \PP\begin{bmatrix} a \\ b \end{bmatrix} = \begin{bmatrix}\rule{0pt}{1.1em}\blank{0.8cm}\\[2pt]\blank{0.8cm}\end{bmatrix}.
\]
Multiplying on the left, $\PP A$ swaps the two \emph{rows} of $A$:
\[
  \PP\begin{bmatrix} 0 & 2 \\ 1 & 3 \end{bmatrix}
  = \begin{bmatrix}\rule{0pt}{1.1em}\blank{0.8cm} & \blank{0.8cm}\\[2pt]\blank{0.8cm} & \blank{0.8cm}\end{bmatrix}.
\]
...
\]
Now the pivots are $\blank{0.7cm}$ and $\blank{0.7cm}$ --- both nonzero, so elimination runs. Here
$\PP A$ is \emph{already} upper triangular, so $U=\begin{bsmallmatrix} 1&3\\0&2 \end{bsmallmatrix}$ and no
elimination step is needed: $L=I$. The factorization is
\[
...
\[
  ...
  \;\Rightarrow\; 2y=2 \Rightarrow y = \blank{0.8cm},\quad x+3y=4 \Rightarrow x = \blank{0.8cm}.
\]
So $(x,y)=(\,\blank{1.4cm}\,)$. Check it in the \emph{original} order: $0(1)+2(1)=2$ \checkmark\ and
...
\[
  ...
  \;\Rightarrow\;
  A^{\tp} = \begin{bmatrix}\rule{0pt}{1.1em}\blank{0.7cm} & \blank{0.7cm}\\[2pt] \blank{0.7cm} & \blank{0.7cm}\\[2pt] \blank{0.7cm} & \blank{0.7cm}\end{bmatrix}.
\]
...
\[
  ...
  \qquad\text{Symmetric: } \blank{1.6cm}
\]

\vspace{2pt}
Two rules we'll reuse. Transpose \textbf{reverses the order} of a product (2.2's socks-and-shoes again):
\[
  (AB)^{\tp} = \blank{1.2cm}\,\blank{1.2cm}\quad\text{(fill in: } B^{\tp}A^{\tp}\text{ or } A^{\tp}B^{\tp}?\text{)}.
\]
...
\begin{enumerate}[leftmargin=1.6em]
  \item Write the swap matrix $\PP$ and compute $\PP A$. \hfill $\PP A=\begin{bmatrix}\ \blank{0.7cm} & \blank{0.7cm}\ \\[2pt] \blank{0.7cm} & \blank{0.7cm} \end{bmatrix}$
  \item Solve $A\xx=\bb$ using $\PP A\xx=\PP\bb$. \hfill $x=\blank{1.1cm}$\quad $y=\blank{1.1cm}$
  \item Write $A^{\tp}$. Is $A$ symmetric? \writeline
\end{enumerate}
\end{practicebox}
