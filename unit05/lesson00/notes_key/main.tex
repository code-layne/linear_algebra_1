%! Setting Up Determinants --- the 2 by 2 Determinant, Area, and Orientation — Unit 5, Lesson 5.0 — Guided Notes (Answer Key)
...
\pageheader{Unit 5, Lesson 5.0}{Guided Notes --- Answer Key}
\namedateperiod
...
\vspace{2pt}
\vocabans{Determinant $\det A=ad-bc$}{one number from the four entries: down-diagonal $ad$ minus up-diagonal $bc$}
\vocabans{Signed area (parallelogram)}{$|\det A|$ is the area of the parallelogram the two columns span}
\vocabans{Orientation (the sign)}{$+$ keeps the plane's turning direction; $-$ flips it (a mirror reflection)}
...
\begin{itemize}\setlength{\itemsep}{1pt}
  \item What \emph{one} measurement tells you how much \emph{bigger} the logo got? \ansline{The area of the new parallelogram --- which is $|\det A|$.}
  \item What would tell you the image was \emph{mirror-flipped}? \ansline{A \emph{negative} determinant --- the sign of $\det A$ records orientation.}
\end{itemize}
\end{hookbox}
...
below have base $3$ and height $2$, so area $={\color{keyred}\mathbf{6}}$, and indeed
$\det\begin{bsmallmatrix} 3 & 1\\ 0 & 2 \end{bsmallmatrix}=(3)(2)-(1)(0)={\color{keyred}\mathbf{6}}$.
...
determinant --- \ans{stays} the same.
\end{notesbox}
...
means the pair is \ans{clockwise (reversed)} --- a mirror \emph{flip}.
...
\[
  ...
  \qquad
  \det\begin{bmatrix} 1 & 3 \\ 2 & 0 \end{bmatrix} = (1)(0)-(3)(2) = {\color{keyred}\mathbf{-6}}.
\]
...
\[
  \det\begin{bmatrix} 2 & 4 \\ 1 & 2 \end{bmatrix} = (2)(2)-(4)(1) = {\color{keyred}\mathbf{0}}.
\]
This is exactly when $A$ is \textbf{not invertible} --- the Unit~2 inverse divides by $ad-bc$, and
you cannot divide by \ans{$0$}. So one number decides everything:
\[
...
factor every area is \ans{multiplied} by.
\end{notesbox}

\begin{practicebox}
Show your work. Remember: $\det\begin{bsmallmatrix} a & b\\ c & d \end{bsmallmatrix}=ad-bc$; area is $|\det|$.
\begin{enumerate}[leftmargin=1.6em, itemsep=4pt]
  \item Compute $\det\begin{bsmallmatrix} 4 & 2\\ 1 & 3 \end{bsmallmatrix}$. \ansline{$(4)(3)-(2)(1)=12-2=10$.}
  \item Find the area of the parallelogram with columns $\begin{bsmallmatrix} 5\\0 \end{bsmallmatrix}$ and $\begin{bsmallmatrix} 2\\3 \end{bsmallmatrix}$. \ansline{$\det=(5)(3)-(2)(0)=15$; area $=|15|=15$.}
  \item Find $x$ so that $\begin{bsmallmatrix} 3 & x\\ 2 & 4 \end{bsmallmatrix}$ has $\det=0$. Is the matrix invertible then? \ansline{$(3)(4)-(x)(2)=12-2x=0\Rightarrow x=6$. Not invertible --- $\det=0$ means the columns are parallel.}
\end{enumerate}
\end{practicebox}

\begin{teachernote}
The spine is Section~1 (the rule $ad-bc$) and Section~2 (that number \emph{is} an area). Sections~3
and~4 are the two payoffs: the \emph{sign} carries orientation (swap $\Rightarrow$ negate), and
$\det=0$ is dependence --- flat box, no inverse (the §2 callback). Most common errors: computing
$ad+bc$, dropping the absolute value when asked for \emph{area}, or reading $\det=0$ as ``no answer''
instead of ``columns parallel / not invertible.'' Section~4's chain
$\det\ne0\Leftrightarrow$ independent $\Leftrightarrow$ invertible is the sentence to land.
\end{teachernote}
