...
\]
So $(x,y)=(\,\ans{$4,\,1$}\,)$ --- the same answer as 2.1 and 2.2, with no inverse in sight.
\end{notesbox}
...
\begin{enumerate}[leftmargin=1.6em]
  \item Find the multiplier, then write $L$ and $U$. \hfill $\ell=\ans{$3$}$
        \ansline{$U=\begin{bsmallmatrix} 1&4\\0&-2 \end{bsmallmatrix}$ (row$_2-3\,$row$_1$), $L=\begin{bsmallmatrix} 1&0\\3&1 \end{bsmallmatrix}$.}
  \item \textbf{Check} $LU=A$.
        \ansline{$\begin{bsmallmatrix} 1&0\\3&1 \end{bsmallmatrix}\begin{bsmallmatrix} 1&4\\0&-2 \end{bsmallmatrix}=\begin{bsmallmatrix} 1&4\\3&10 \end{bsmallmatrix}=A$ \checkmark}
  \item Solve $A\xx=\bb$ by two sweeps ($L\cc=\bb$, then $U\xx=\cc$). \hfill $x=\ans{$2$}$\quad $y=\ans{$1$}$
        \ansline{Forward: $c_1=6$, $3(6)+c_2=16\Rightarrow c_2=-2$, so $\cc=\begin{bsmallmatrix} 6\\-2 \end{bsmallmatrix}$. Back: $-2y=-2\Rightarrow y=1$, $x+4=6\Rightarrow x=2$.}
\end{enumerate}
\end{practicebox}

\begin{teachernote}
The through-line: $A=LU$ is elimination \emph{saved}. $U$ is the 2.1 result; $L$ is the 2.2
undo-matrices $E^{-1}$ gathered up, so its below-diagonal entries are just the multipliers (with a
\emph{plus} sign --- the most common slip is writing $-\ell$). Emphasize \S3: the forward sweep
reproduces ``update the right-hand side'' (watch $\cc=\begin{bsmallmatrix} 7\\1 \end{bsmallmatrix}$ match
2.1), and the back sweep is ordinary back-substitution. \S4 is the concept behind the section title:
factor once, then two cheap sweeps per order --- how real solvers work.
\end{teachernote}
