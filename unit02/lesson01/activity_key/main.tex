...
\begin{work}
  (4x+3y)-2(2x+y) &= 17-14 \\
                y &= 3
\end{work}
  \item Back-substitute into row~1 to find $x$. \; $x=\ans{$2$}$
\begin{work}
  2x+3 &= 7 \\
     x &= 2
\end{work}
...
\begin{work}
  \text{swap rows:}\quad 4x+3y &= 17 \\
                          2y &= 6 \\
                           ...
                           x &= 2
\end{work}
\ansline{The pivot slot (row~1's $x$-coefficient) is $0$, so there is no pivot to clear the
column. Swapping the rows puts a nonzero there. Solution $(2,3)$.}
...
\begin{work}
  (2x+4y)-2(x+2y) &= 9-8 \\
                0 &= 1
\end{work}
        \ansline{\textbf{No solution.} The last line $0=1$ is impossible, so the lines are
...
\begin{work}
  (2x+4y)-2(x+2y) &= 8-8 \\
                0 &= 0
\end{work}
        \ansline{\textbf{Infinitely many.} The last line $0=0$ says equation~2 is just twice
        equation~1 (same line), so every point on it solves the system.}
\end{enumerate}
\end{tcolorbox}
