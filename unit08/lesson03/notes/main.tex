%! Minimizing Loss by Gradient Descent — Unit 8, Lesson 8.3 — Guided Notes
...
\begin{itemize}\setlength{\itemsep}{1pt}
  \item If ``good weights'' means ``small loss,'' where in the landscape do we want to end up? \writeline
  \item You are in a valley in thick fog and can feel only the \emph{slope} under your feet. How would you walk to the bottom? \writeline
\end{itemize}
\end{hookbox}
...
\[
  L'(w)=2w-8.
\]
At $w=0$: $\;L'(0)=2(0)-8=\blank{1.0cm}$. The slope is \emph{negative}, so the loss is still falling as $w$
\emph{increases} --- downhill is to the \textbf{right} (toward $w=4$). At the very bottom $w=4$, the slope is
$L'(4)=\blank{0.8cm}$: flat ground, nowhere lower to go.
...
\begin{tikzpicture}[scale=0.92]
  ...
  \begin{scope}[shift={(0,0)}]
    ...
    % ball on the left wall at w=1 (x=0.6,y=0.81)
    \fill[burgundy] (0.6,0.81) circle (3pt);
    % short tangent (uphill) segment through the ball
  ...
  \begin{scope}[shift={(6.4,0)}]
    ...
    \draw[ultra thick, royalblue, smooth]
      plot coordinates {(0,1.44)(0.6,0.81)(1.2,0.36)(1.8,0.09)(2.4,0)(3.0,0.09)(3.6,0.36)(4.2,0.81)(4.8,1.44)};
    % iterates w=0,2,3,3.5 -> x=0,1.2,1.8,2.1 ; y=1.44,0.36,0.09,0.02
...
$0$ & $-8$ & $0-0.25(-8)$ & $\blank{1.0cm}$\\
$2$ & $-4$ & $2-0.25(-4)$ & $\blank{1.0cm}$\\
$3$ & $-2$ & $3-0.25(-2)$ & $\blank{1.0cm}$\\
\end{tabular}
\end{center}
...
\[
  (0,0)-0.25\,[-2,-6]=\big(\ \blank{0.7cm},\ \blank{0.7cm}\ \big),
\]
...
\begin{enumerate}[leftmargin=1.6em, itemsep=4pt]
  \item Take one step from $w=6$. (Find $L'(6)$, then $w-s\,L'(6)$.) Which way did $w$ move, and toward what? \writeline
  \item Redo one step from $w=0$ but with a \emph{big} learning rate $s=1$. Where does $w$ land, and what went wrong? \writeline
  \item Two weights: $\nabla L=[\,2(w_1-1),\,2(w_2-3)\,]$. Take one step ($s=0.25$) from $(2,0)$. \writeline
\end{enumerate}
\end{practicebox}
