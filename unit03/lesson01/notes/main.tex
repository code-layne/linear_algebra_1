%! Vector Spaces and Subspaces — Unit 3, Lesson 3.1 — Guided Notes
...
\vspace{2pt}
\termblanklong{Vector space}
\termblanklong{Subspace}
\termblanklong{Subspace requirement}
\termblanklong{Zero subspace $\{\zero\}$}
...
\begin{itemize}\setlength{\itemsep}{1pt}
  \item Can it reach the sum $\bb_1+\bb_2$? What single control setting $\xx$ gets there? \writeline
  \item Which rule about $A\xx$ makes your answer work? \writeline
\end{itemize}
\end{hookbox}
...
\end{itemize}
The model example is $\mathbb{R}^n$ = the set of all vectors with $n$ real \blank{2.4cm}. Every rule
above just says arithmetic in $\mathbb{R}^n$ behaves normally.
\end{notesbox}

\begin{notesbox}{2. Subspaces, Made Precise}
A \textbf{subspace} is a subset of $\mathbb{R}^n$ that is \emph{itself} a \blank{2.6cm}. Here is the
...
\textbf{not} enough --- a set can contain $\zero$ and still fail rule \blank{1.0cm}, as we'll see next.
\end{notesbox}
...
\begin{itemize}[leftmargin=1.5em, itemsep=1pt]
  \item In $\mathbb{R}^2$: just $\{\zero\}$, any \blank{2.8cm} through the origin, and all of $\mathbb{R}^2$.
...
$\vv+\ww=\begin{bsmallmatrix} 2\\2 \end{bsmallmatrix}$ is on \blank{2.2cm} axis --- so $S$ fails rule
...
\begin{itemize}[leftmargin=1.5em, itemsep=2pt]
  \item \textbf{Addition:} $\bb_1+\bb_2=A\xx_1+A\xx_2=A(\,\blank{1.8cm}\,)$, so $\bb_1+\bb_2$ is
        reachable --- it is $A$ times the control setting $\xx_1+\xx_2$.
  \item \textbf{Scaling:} $c\,\bb_1=c(A\xx_1)=A(\,\blank{1.4cm}\,)$, so $c\,\bb_1$ is reachable too.
\end{itemize}
...
\begin{enumerate}[leftmargin=1.6em, itemsep=4pt]
  \item All $\begin{bsmallmatrix} x\\y \end{bsmallmatrix}$ with $x+y=0$. \writeline
  \item All $\begin{bsmallmatrix} x\\y \end{bsmallmatrix}$ with $xy=0$ (the union of the axes). \writeline
  \item The column space of $A=\begin{bsmallmatrix} 1 & 2 \\ 0 & 1 \end{bsmallmatrix}$. \writeline
\end{enumerate}
\end{practicebox}
