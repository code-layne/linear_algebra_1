%! Least Squares Approximations — Unit 4, Lesson 4.3 — Guided Notes (Answer Key)
...
\vspace{2pt}
\vocabans{Least-squares solution $\hat{\xx}$}{the weights $(C,D)$ that make $A\hat{\xx}$ closest to $\bb$; they solve the normal equations $A^{\T}A\hat{\xx}=A^{\T}\bb$}
\vocabans{Best-fit line $y=C+Dt$}{the line built from $\hat{\xx}$; it comes nearest to the data, though it need not pass through any point}
\vocabans{Residual $\ee=\bb-\pp$}{the vertical miss between each data value and the line; perpendicular to the columns of $A$ ($A^{\T}\ee=\zero$)}
\end{vocabbox}
...
\begin{itemize}\setlength{\itemsep}{1pt}
  \item Can a single straight line pass \emph{exactly} through all four measurements? \ansline{Almost never --- four points rarely land on one line, so the system $A\xx=\bb$ has no exact solution.}
  \item If no line hits every point, what should we mean by the \emph{best} line? \ansline{The line whose total (squared) vertical miss is smallest --- the projection of the data onto the plane of possible lines.}
\end{itemize}
\end{hookbox}
...
\ans{no} exact solution. Instead of giving up, we ask for the point $\pp=A\hat{\xx}$ \emph{in} the
...
\[
  ...
  C={\color{keyred}\mathbf{2}},\quad D={\color{keyred}\mathbf{2}}
  \quad\Longrightarrow\quad \hat{\xx}=\begin{bsmallmatrix} 2\\ 2 \end{bsmallmatrix}.
\]
\textbf{Best-fit line:} $y={\color{keyred}\mathbf{2}}+{\color{keyred}\mathbf{2}}\,t$, i.e. $y=2+2t$.
\vspace{-4pt}
\ansline{Reduce $4C+6D=20$, $6C+14D=40$ to $2C+3D=10$, $3C+7D=20$; eliminate $C$: $5D=10$, so $D=2$ and $C=2$.}
\end{notesbox}
...
\]
They are perpendicular to both columns: $(1,1,1,1)\cdot\ee={\color{keyred}\mathbf{0}}$ and
$(0,1,2,3)\cdot\ee={\color{keyred}\mathbf{0}}$. So $\ee$ lands in the left nullspace $N(A^{\T})$, the orthogonal
...
\begin{enumerate}[leftmargin=1.6em, itemsep=6pt]
  \item The three points $(0,3),(1,4),(2,5)$ happen to lie \emph{exactly} on one line. What line is it,
        ...
        solution.)
        \ansline{$y=3+t$: at $t=0,1,2$ it gives $3,4,5$, matching the data exactly. Every residual is $0$ ($\ee=\zero$), so $A\hat{\xx}=\bb$ has an exact solution and no projection is needed.}
  \item In a least-squares fit, what does a residual $e_i$ measure, and why do we make the sum of their
        \emph{squares} small rather than just their sum?
        \ansline{$e_i=y_i-(C+Dt_i)$ is how far the data point sits above ($+$) or below ($-$) the line. Plain residuals cancel (positives and negatives sum to $0$), so we square first: squares are all $\ge 0$, cannot cancel, and penalize big misses more --- minimizing $\sum e_i^2$ is the same as making the error vector $\ee$ as short as possible.}
\end{enumerate}
\end{practicebox}

\begin{teachernote}
The one idea: least squares \emph{is} the Lesson~4.2 projection, now applied to data. Row $i$ of
$A\xx=\bb$ is the wish ``$C+Dt_i=y_i$''; with more points than unknowns those wishes conflict, so we
project $\bb$ onto $C(A)$ and read off $\hat{\xx}=(C,D)$. Emphasize the two columns of $A$: the constant
column $(1,1,1,1)$ and the time column $(t_i)$ --- their dot products build $A^{\T}A$ and $A^{\T}\bb$
mechanically (sums of $1$, of $t$, of $t^2$, of $y$, of $ty$). Common slips: forgetting the column of
$1$s (that column \emph{is} the intercept $C$); trying to solve $A\xx=\bb$ directly (no solution --- that
is the whole point); adding raw residuals instead of squares. The residual $\ee=(1,-1,-1,1)$ landing in
$N(A^{\T})$ is the Lesson~4.1 payoff. Next lesson (4.4) makes $A^{\T}A$ trivial when the columns are
orthonormal.
\end{teachernote}
